...
- **`src/backends/claude_proxy.rs`**: `ClaudeProxyInferenceService`, feature-gated behind `claude-proxy` (off by default, and inert even when compiled in — `new()` fails fast on an empty `api_key`). Real HTTPS proxy to Anthropic's Messages API (`POST /v1/messages`); no local model, no weights — a stateless remote backend behind the same `InferenceService` trait. **Explicitly `v0.2+`/experimental, unruled**: decision memos #2 (remote fallback scope) and #9 (hub relay auth) in `ai-kit-unified-design.md` are still open — sending prompt text to a third-party cloud API needs a security-engineer/DJ privacy ruling before this ships beyond a dev POC. **Limitations** (deliberate, same trait shape as `candle`): no streaming, no tool use/function calling, no multi-turn conversation — single-shot prompt→completion only; `quantization` in the response is always `"n/a (cloud proxy)"` (no quantization concept for a cloud proxy); `cache_stats()` returns a zeroed stub (nothing loaded/cached locally). Cites Apple's WWDC 2026 Foundation Models framework (provider-swappable `LanguageModelSession`-style call site) as precedent for the *pattern* only — see the module's own doc comment for a caveat that a live web search could not fully corroborate the specific third-party-provider claim beyond WWDC 2026 coverage of Gemini-powered Siri.
